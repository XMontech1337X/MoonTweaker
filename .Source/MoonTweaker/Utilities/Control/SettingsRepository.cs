...
namespace MoonTweaker.Utilities.Control
{
    [StructLayout(LayoutKind.Sequential)]
    internal readonly struct StoragePaths
    {
        internal static string Config = string.Empty;
        internal static string FolderLocation => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), @"MoonTweaker");
        ...
        internal static string RegistryLocation = @"HKEY_CURRENT_USER\Software\MoonTweaker";
    }
    ...
    internal sealed class SettingsRepository
    {
        ...
        internal void СheckingParameters()
        {
            ...
            if (isRegistryEmpty)
            {
                foreach (var subkey in defaultRegValues)
                    RegistryHelp.Write(Registry.CurrentUser, @"Software\MoonTweaker", subkey.Key, subkey.Value, RegistryValueKind.String);
            }
            else
            {
        ...
        {
            RegistryHelp.Write(Registry.CurrentUser, @"Software\MoonTweaker", subkey, value.ToString(), RegistryValueKind.String);
        ...
        internal static void SaveFileConfig()
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    iniManager.Write("MoonTweaker", "Author", "XMontech1337X");
                    iniManager.Write("MoonTweaker", "Release", currentRelease);
        ...
        internal static void OpenFileConfig()
        {
            ...
            if (iniManager.GetKeysOrValue("MoonTweaker", false).Contains("XMontech1337X") && iniManager.GetKeysOrValue("MoonTweaker").Contains("Release"))
            {
        ...
        internal static void SelfRemoval()
        {
            try
            {
                RegistryHelp.DeleteFolderTree(Registry.CurrentUser, @"Software\MoonTweaker");
                RegistryHelp.DeleteFolderTree(Registry.LocalMachine, @"SOFTWARE\Microsoft\Tracing\MoonTweaker_RASAPI32");
                RegistryHelp.DeleteFolderTree(Registry.LocalMachine, @"SOFTWARE\Microsoft\Tracing\MoonTweaker_RASMANCS");
                ...
                CommandExecutor.RunCommand($"/c taskkill /f /im {currentName} & choice /c y /n /d y /t 3 & del {new FileInfo(new Uri(Assembly.GetExecutingAssembly().CodeBase).LocalPath).Name} & " +
                    @$"rd /s /q {StoragePaths.FolderLocation} & rd /s /q {Environment.SystemDirectory}\config\systemprofile\AppData\Local\MoonTweaker");
            }
            catch (Exception ex) { Debug.WriteLine(ex.Message); }
        }
